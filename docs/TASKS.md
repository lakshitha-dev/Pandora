...
| Task | Owner | Status | Blockers |
|---|---|---|---|
| 🔴 **Stub every §1 endpoint returning the exact contract payloads — unblocks Nipuna in 30 minutes.** A FastAPI stub returning a dict is ~3 lines. | Manujaya | todo | API contract frozen |
| 🔴 CORS for `localhost:3000` + the Static Web App origin — configure now, don't debug it at 2am | Manujaya | todo | Scaffold |
| 🟡 FastAPI scaffold: `app/{api,core,db,schemas,services}/`, `requirements.txt`, Uvicorn entrypoint | Manujaya | todo | Folder structure |
| 🟡 Pydantic `Settings` class reading env vars — validated at startup, fails loudly on a missing key. **No scattered `os.getenv()`.** | Manujaya | todo | Scaffold |
| 🟡 Pydantic schemas mirroring every §1 request/response shape | Manujaya | todo | API contract frozen |
| 🟡 SQLAlchemy 2.0 models: `documents`, `answers`, `agent_actions`, `conversations` | Manujaya | todo | Scaffold |
| 🟡 Alembic init + first migration, applied to Azure PostgreSQL | Manujaya | todo | Models + Postgres provisioned |
| 🟡 Supabase JWT validation dependency (`python-jose`) — signature + expiry, extract user ID | Manujaya | todo | Supabase JWT secret |
...
| 🟡 `GET/DELETE /api/v1/documents` — **delete must remove Postgres row AND AI Search vectors** | Manujaya | todo | Models |
| 🔥 `POST /api/v1/ask` — orchestrate `/rag/query` → `/agent/run`, join `document_name` from Postgres, assemble the response | Manujaya | todo | Agent client |
| 🔥 `POST` + `PATCH /api/v1/agent-actions` — persist, approve/reject/complete, set `resolved_at` | Manujaya | todo | Models |
| 🟡 `GET /api/v1/agent-actions` with status/type filters and paging | Manujaya | todo | Models |
...
| 🟢 `422 no_documents_indexed` guard before any LLM call | Manujaya | todo | `/ask` working |
